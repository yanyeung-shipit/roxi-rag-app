...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("enhanced_65percent.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("Enhanced65PercentProcessor")

# Constants
TARGET_PERCENTAGE = 65.0
CHECKPOINT_FILE = "enhanced_65percent_checkpoint.json"
BATCH_SIZE = 3

def run_enhanced_processor():
    """
    Main function to run the enhanced processor to 65%.
    """
    try:
        # Import the enhanced batch processor
        from enhanced_batch_processor import BatchProcessor
        
        # Create processor with target of 65%
        ...
        traceback.print_exc()
        print(f"\nError: {str(e)}")
        print("Check enhanced_65percent.log for details.")
        return 1

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Enhanced chunk processor to 65% completion")
