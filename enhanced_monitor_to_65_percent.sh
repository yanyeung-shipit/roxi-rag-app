#!/bin/bash
# Enhanced Monitor Script for 65% Target
# This script monitors and restarts the enhanced_process_to_65_percent.py script
# if it stops, ensuring continuous processing until the target is reached.

# Log file path
LOG_FILE="enhanced_65percent_monitor.log"
...
is_processor_running() {
    pgrep -f "python.*enhanced_process_to_65_percent.py" > /dev/null
    return $?
}
...
start_processor() {
    log_message "Starting enhanced processor to 65%..."
    nohup python enhanced_process_to_65_percent.py --batch-size 3 > enhanced_65percent.log 2>&1 &
    sleep 5
    if is_processor_running; then
        log_message "Processor started successfully with PID $(pgrep -f 'python.*enhanced_process_to_65_percent.py')"
    else
...
log_message "Starting enhanced monitor script for 65% target"
...
while true; do
    ...
    # If we've reached or exceeded 65%, exit
    if (( $(echo "$current_progress >= 65.0" | bc -l) )); then
